...
# The first section of the code prompts the user to either create an account, or log in to their account
# To create an account, the user needs to enter a new username and password
# To log in to their account, the user needs to enter the correct username and password (i.e. the same username/password that they entered when creating their account)
# Users that are successful in logging into their account can then access the Travel Buddy vacation planner
    ...
    if ((month_input > 12) or (month_input < 1)):
      ...
      while weather != "E":
            ...
            if has_records:
              ...
              # the visual is an interactive graph that shows average temperature for the chosen city
              # the x-axis represents the months of the year from Jan to Dec
              # the y-axis represents the years over which temperature data is collected or available from 1995 to 2020
                ...
                if visual != "no":
                  ...
                  #https://matplotlib.org/stable/gallery/images_contours_and_fields/colormap_interactive_adjustment.html#interactive-adjustment-of-colormap-range
                    ...
                    # handling NaN values by replacing them with the mean of non-NaN values
